...
private:
  ...
  void EmitPrintMachineOperand(raw_ostream &o);
...
static bool LessOperandByPrintMethod(const Record *LHS, const Record *RHS) {
  return LHS->getValueAsString("PrintMethod") <
         RHS->getValueAsString("PrintMethod");
}

/// Generate the code for the "printMachineOperand" method implementation.
/// Needs AsmWriterInst information.
void AsmWriterEmitter::EmitPrintMachineOperand(raw_ostream &O) {
  Record *AsmWriter = Target.getAsmWriter();
  StringRef TGName = Target.getName();
  std::string ClassName = AsmWriter->getValueAsString("AsmWriterClassName");
  unsigned PassSubtarget = AsmWriter->getValueAsInt("PassSubtarget");

  O << "\n#ifdef PRINT_MACHINE_OPERAND\n";
  O << "#undef PRINT_MACHINE_OPERAND\n\n";
  O <<
  "/// printMachineOperand - This method is automatically generated by tablegen\n"
  "/// from the instruction set description.\n"
    "void " << TGName << ClassName
            << "::printMachineOperand(const MCInst *MI, "
            << "unsigned OpType, unsigned OpNo, const MCSubtargetInfo &STI, "
            << "raw_ostream &O) {\n";

  std::vector<Record *> CGOperands;
  {
    ...
    CGOperands.resize(FoundOperands.size());
    std::copy(FoundOperands.begin(), FoundOperands.end(), CGOperands.begin());
  }

  std::sort(CGOperands.begin(), CGOperands.end(), LessRecordByID());
  std::stable_sort(CGOperands.begin(), CGOperands.end(),
                   LessOperandByPrintMethod);

  O << "  switch(OpType) {\n";
  O << "  default:\n"
    << "    llvm_unreachable(\"Trying to print invalid operand type!\");\n"
    << "    break;\n";
  for (unsigned oi = 0, oe = CGOperands.size(); oi != oe; ++oi) {
    O << "  case " << TGName << "::OpTypes::" << CGOperands[oi]->getName()
      << ":\n";
    const std::string PrintMethod =
      CGOperands[oi]->getValueAsString("PrintMethod");
    // Fallthrough if we have the same print method.
    if ((oi + 1) < oe &&
        CGOperands[oi + 1]->getValueAsString("PrintMethod") == PrintMethod)
      continue;
    O << "    " << PrintMethod << "(MI, OpNo, "
      << (PassSubtarget ? "STI, " : "") << "O); break;\n";
  }
  O << "  }\n";
  O << "}\n\n";
  O << "#endif // PRINT_MACHINE_OPERAND\n";
}
